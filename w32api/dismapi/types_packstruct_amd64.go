...
func (s *DismFeatureEx) ToPackedByte() []byte {
	...
	b.Write(unsafe.Slice((*byte)(unsafe.Pointer(&s.DismFeature.FeatureName)), 8))
	b.Write(unsafe.Slice((*byte)(unsafe.Pointer(&s.DismFeature.State)), 4))
	b.Write(unsafe.Slice((*byte)(unsafe.Pointer(&s.CustromPropertyInXML)), 8))
...
func ToStruct[P PackedStruct](buf []byte) (P, error) {
	...
	case DismFeatureEx:
		...
		result = DismFeatureEx{
			...
			CustromPropertyInXML: *(**uint16)(unsafe.Pointer(&buf[12])),
		}
	case DismFeatureInfo:
		if GetPackedSize(sst) != len(buf) {
			return st, fmt.Errorf("the size of buffer does not match the size of struct")
