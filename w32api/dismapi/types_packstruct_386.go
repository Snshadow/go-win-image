// Code generated by "mkpackstruct -parsed types.go types.go"; DO NOT EDIT.
...
func (s *DismFeatureEx) ToPackedByte() []byte {
	var b bytes.Buffer

	b.Write(unsafe.Slice((*byte)(unsafe.Pointer(&s.DismFeature.FeatureName)), 4))
	b.Write(unsafe.Slice((*byte)(unsafe.Pointer(&s.DismFeature.State)), 4))
	b.Write(unsafe.Slice((*byte)(unsafe.Pointer(&s.CustromPropertyInXML)), 4))
...
type PackedStruct interface {
	DismAppxPackage | DismCapability | DismCapabilityInfo | DismCustomProperty | DismDriver | DismDriverPackage | DismFeature | DismFeatureEx | DismFeatureInfo | DismImageInfo | DismLanguage | DismMountedImageInfo | DismPackage | DismPackageInfo | DismPackageInfoEx | DismString | DismWimCustomizedInfo
}
...
func ToStruct[P PackedStruct](buf []byte) (P, error) {
	...
	case DismFeatureEx:
		...
		result = DismFeatureEx{
			DismFeature: DismFeature{
				FeatureName: *(**uint16)(unsafe.Pointer(&buf[0])),
				State:       *(*DismPackageFeatureState)(unsafe.Pointer(&buf[4])),
			},
			CustromPropertyInXML: *(**uint16)(unsafe.Pointer(&buf[8])),
		}
	case DismFeatureInfo:
		if GetPackedSize(sst) != len(buf) {
			return st, fmt.Errorf("the size of buffer does not match the size of struct")
